            ...
            foreach ($data as $record) {
                for ($i = 0; $i < count($this->_select); $i++) {
                    if ($this->_formatter[$i] == 'int') {
                        ...
                        if ($record->{$this->_select[$i]} == '0000-00-00' || $record->{$this->_select[$i]} == '') {
                            $row[$this->_select[$i]] = '-';
                        } else {
                            $row[$this->_select[$i]] = get_general_date($record->{$this->_select[$i]});
                        }
                    }elseif ($this->_formatter[$i] == 'cek_expired') {
                        if ($record->{$this->_select[$i]} == '0000-00-00' || $record->{$this->_select[$i]} == '') {
                            $row[$this->_select[$i]] = '-';
                        } else {
                            if($record->{$this->_select[$i]} > date('Y-m-d')){
                                $row[$this->_select[$i]] = '<b style="color:green">'.get_general_date($record->{$this->_select[$i]}).'</b>';
                            }else{
                                $row[$this->_select[$i]] = '<b style="color:red">'.get_general_date($record->{$this->_select[$i]}).'</b>';
                            }
